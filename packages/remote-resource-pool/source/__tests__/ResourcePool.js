...
describe('ResourcePool', () => {
  ...
  describe('When adding target', () => {
    ...
    describe('When adding new', () => {
      ...
      beforeEach(() => {
        ...
        pool.addEventListener(ResourcePool.RESOURCE_ADDED, listener);
      ...
      it(`should fire event "${ResourcePool.RESOURCE_ADDED}"`, () => {
      ...
      describe('When adding already existing', () => {
        ...
        it(`should not fire event "${ResourcePool.RESOURCE_ADDED}"`, () => {
  ...
  describe('When removing target', () => {
    ...
    beforeEach(() => {
      ...
      pool.addEventListener(ResourcePool.RESOURCE_REMOVED, listener);
    ...
    it(`should fire event "${ResourcePool.RESOURCE_REMOVED}"`, () => {
      expect(listener).to.be.calledOnce;
    });
  ...
  describe('When clearing pool', () => {
    ...
    beforeEach(() => {
      ...
      pool.addEventListener(ResourcePool.POOL_CLEAR, beforeListener);
      pool.addEventListener(ResourcePool.POOL_CLEARED, afterListener);
      pool.clear();
    });

    it(`should fire "${ResourcePool.POOL_CLEAR}" event`, () => {
      expect(beforeListener).to.be.calledOnce;
    });

    it(`"${ResourcePool.POOL_CLEAR}" event should target to own pool`, () => {
      expect(beforeListener.getCall(0).args[0].data).to.be.equal(pool);
    });

    it(`should fire "${ResourcePool.POOL_CLEAR}" event before destroying resources`, () => {
      expect(beforeActive).to.be.true;
      expect(beforeExists).to.be.true;
    });

    it(`should fire "${ResourcePool.POOL_CLEARED}" event`, () => {
      expect(beforeListener).to.be.calledOnce;
    });

    it(`"${ResourcePool.POOL_CLEARED}" event should target to own pool`, () => {
      expect(afterListener.getCall(0).args[0].data).to.be.equal(pool);
    });

    it(`should fire "${ResourcePool.POOL_CLEARED}" event after resources destroyed`, () => {
  ...
  describe('When destroying pool', () => {
    ...
    beforeEach(() => {
      ...
      pool.addEventListener(ResourcePool.POOL_CLEARED, clearedListener);
      pool.addEventListener(ResourcePool.POOL_DESTROYED, destroyedListener);
      pool.destroy();
    });
    ...
    it(`should fire "${ResourcePool.POOL_DESTROYED}" event`, () => {
